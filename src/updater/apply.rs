...
const STAGED_BINARY_FILENAME: &str = ".sumanmovies_update_staged.exe";
const HELPER_SCRIPT_FILENAME: &str = "sumanmovies_update_helper.bat";
...
pub fn is_writable(exe_path: &Path) -> bool {
    ...
    let test_file = parent.join(format!(".sumanmovies_write_test_{}", std::process::id()));
    ...
    match env {
        InstallationEnvironment::Homebrew => {
            Ok(SelfUpdateOutcome::RequiresManualUpgrade(
                "This installation is managed by Homebrew. Run: brew upgrade sumanmovies".to_string(),
            ))
        }
        ...
        InstallationEnvironment::Snap => {
            Ok(SelfUpdateOutcome::RequiresManualUpgrade(
                "Running inside Snap. Please update via: sudo snap refresh sumanmovies".to_string(),
            ))
        }
...
mod tests {
    ...
    fn persistent_staging_path_lives_beside_executable() {
        let exe = Path::new("/opt/tools/sumanmovies.exe");
        let staged = persistent_staging_path(exe);
        assert_eq!(staged.parent(), Some(Path::new("/opt/tools")));
        assert!(staged.to_string_lossy().contains("sumanmovies_update_staged"));
    }

    #[test]
    fn stale_artifacts_cover_staging_and_helper() {
        let exe = Path::new("C:/App/sumanmovies.exe");
        let paths = stale_update_artifacts(exe);
        assert_eq!(paths.len(), 2);
        assert!(
            paths[0]
                .to_string_lossy()
                .contains("sumanmovies_update_staged")
        );
        assert!(
            paths[1]
                .to_string_lossy()
                .contains("sumanmovies_update_helper.bat")
        );
    }
    ...
    fn detects_scoop_managed_environment() {
        let win_path =
            Path::new("C:\\Users\\user\\scoop\\apps\\sumanmovies\\0.1.23\\sumanmovies.exe");
        assert!(is_scoop_managed(win_path));
        assert_eq!(detect_environment(win_path), InstallationEnvironment::Scoop);

        let shim_path = Path::new("C:\\Users\\user\\scoop\\shims\\sumanmovies.exe");
    ...
    fn helper_script_waits_moves_and_cleans_up() {
        let script = render_helper_script(
            Path::new("C:\\App\\update dir\\.sumanmovies_update_staged.exe"),
            Path::new("C:\\App\\sumanmovies.exe"),
            4242,
        );
        ...
        assert!(script.contains(
            "move /y \"C:\\App\\update dir\\.sumanmovies_update_staged.exe\" \"C:\\App\\sumanmovies.exe\" >NUL 2>&1"
        ));
        assert!(script.contains(":move_loop"));
        assert!(script.contains("if %attempts% LSS 5 ("));
        assert!(script.contains(
            "if exist \"C:\\App\\update dir\\.sumanmovies_update_staged.exe\" del /f /q \"C:\\App\\update dir\\.sumanmovies_update_staged.exe\""
        ));
        assert!(script.contains("start \"\" \"C:\\App\\sumanmovies.exe\""));
    ...
    fn cleanup_removes_only_known_artifacts() {
        ...
        let exe = dir.join("sumanmovies.exe");
