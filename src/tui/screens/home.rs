...
pub fn landing_split(
    ...
    let compact_logo = tier.is_compact() || (tv_mode && area.width < 114) || area.width < 111;
...
pub(crate) fn render_landing_deck(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    if !has_cw && !has_fav && tab != crate::tui::state::HomeDeckTab::Discover {
        return;
    }
    ...
    let (active_style, inactive_style, hint_key_style, sep_style) =
        if modal_active {
            (
                theme.muted,
                theme.muted,
    ...
    if tab == crate::tui::state::HomeDeckTab::Discover {
        ...
        block = block.title(
            Line::from(vec![Span::styled("(Tab) ", hint_key_style)]).alignment(Alignment::Right),
        );
    }
    ...
    let list_items: Vec<ListItem> = match tab {
        crate::tui::state::HomeDeckTab::Discover => presets
            .iter()
            .map(|&(title, desc)| {
...
pub(crate) fn build_landing_subtitle_line<'a>(
    width: u16,
    theme: &'a Theme,
    modal_active: bool,
) -> Line<'a> {
    let tagline = "Cinema.SumanOnline.Com";
    ...
    let tagline_style = if modal_active {
        theme.muted
    } else {
        theme.subtext1
    };
    ...
    if total_w >= tag_len + ver_len + 2 {
        let left_pad = (total_w.saturating_sub(tag_len)) / 2;
        let tagline_end = left_pad + tag_len;
        let version_start = total_w.saturating_sub(ver_len);
        if version_start >= tagline_end {
            let mid_pad = version_start - tagline_end;
            return Line::from(vec![
                Span::raw(" ".repeat(left_pad)),
                Span::styled(tagline, tagline_style),
                Span::raw(" ".repeat(mid_pad)),
                Span::styled(version_str, version_style),
            ]);
        }
    }
    ...
    } else if total_w >= ver_len {
        let left_pad = total_w.saturating_sub(ver_len);
        Line::from(vec![
            Span::raw(" ".repeat(left_pad)),
            Span::styled(version_str, version_style),
        ])
...
pub fn draw(frame: &mut Frame, area: Rect, state: &mut AppState, theme: &Theme) {
        ...
        || (view == SearchViewState::Editing && state.search_results.is_empty())
    {
        ...
        let logo_text = if basic_terminal
            || tier.is_compact()
            || (state.is_tv_mode && area.width < 114)
            || area.width < 111
        {
            if state.is_tv_mode {
                "█▀▀ █ █ █▄ ▄█ █▀█ █▄ █ █▄ ▄█ █▀█ █ █ █ █▀▀ █▀▀ TV\n▄██ █▄█ █ ▀ █ █▀█ █ ▀█ █ ▀ █ █▄█ ▀▄▀ █ ██▄ ▄██".to_string()
...
mod tests {
    ...
    fn test_landing_deck_continue_watching_rendering() {
        ...
        assert!(rendered.contains("Continue Watching") || rendered.contains("Resume"));
        assert!(rendered.contains("Favorites"));
    ...
    fn test_home_banner_rendered_when_no_update_modal() {
        ...
        assert!(content.contains("Cinema.SumanOnline.Com"));
    }

    #[test]
    fn test_build_landing_subtitle_line_layouts() {
        let theme = Theme::mocha();

        // Wide layout (107 cols)
        let line_wide = build_landing_subtitle_line(107, &theme, false);
        let text_wide: String = line_wide.spans.iter().map(|s| s.content.as_ref()).collect();
        assert!(text_wide.contains("Cinema.SumanOnline.Com"));
        assert!(text_wide.contains(&format!("v{}", env!("CARGO_PKG_VERSION"))));
        assert_eq!(text_wide.chars().count(), 107);
    ...
    #[test]
    fn test_large_banner_not_cropped() {
