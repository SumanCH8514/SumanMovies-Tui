...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --help|-h)
            cat << 'EOF'
SumanMovies-TUI Installer

USAGE:
    curl -fsSL https://raw.githubusercontent.com/SumanCH8514/SumanMovies-Tui/main/install.sh | bash -s -- [OPTIONS]
    ./install.sh [OPTIONS]
...
if [ "$IS_TERMUX" -eq 1 ]; then
    ...
    FILE="SumanMovies_macOS_Universal.tar.gz"
    PLATFORM_NAME="macOS (Universal)"
elif [ "$OS" = "Linux" ]; then
    if [ "$ARCH" = "x86_64" ]; then
        FILE="SumanMovies_Linux_x64.tar.gz"
        PLATFORM_NAME="Linux (x86_64)"
    elif [ "$ARCH" = "aarch64" ] || [ "$ARCH" = "arm64" ]; then
        FILE="SumanMovies_Linux_arm64.tar.gz"
