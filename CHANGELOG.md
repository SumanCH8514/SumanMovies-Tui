...
### Added
- **SumanMovies Branding & Visual Identity**:
  - Implemented custom ASCII art banner for `SUMANMOVIES` with dynamic terminal responsiveness.
  - Updated all window headers, status lines, and executable aliases (`sumanmovies`, `SumanMovies`).
- **NPM Package & Instant CLI Distribution (`npm i -g sumanmovies` / `npx sumanmovies`)**:
  - Added self-contained Node.js CLI launcher (`npm/bin/sumanmovies.js`) and cross-platform prerequisite installer (`npm/lib/installer.js`).
  - Automated zero-config provisioning of portable `mpv` player on Windows so first-time users can stream immediately with zero manual dependency setup.
  - Published to NPM registry under [`sumanmovies`](https://www.npmjs.com/package/sumanmovies).
- **Concurrent Poster Prefetching & Full Grid Artwork**:
  - Replaced single-item lazy poster loading with a 16-worker concurrent asynchronous prefetch queue in `src/tui/app/search.rs`.
  - Posters for all search result items and discover grids now fetch and render concurrently regardless of item selection.
  - Added true-color Unicode Halfblock graphics fallback for terminals without native Kitty or Sixel graphics protocols.

### Fixed
- **4KHDHub & HubCloud Player Crash (`Player exited with status code 1`)**:
  - Resolved fatal player startup crashes when streaming from 4KHDHub and HubCloud mirrors.
  - Removed malformed `--ytdl-raw-options` key-value splitting on User-Agent strings with commas (`(KHTML, like Gecko)`).
  - Streamlined player invocation in `src/player.rs` to rely exclusively on native `--user-agent`, `--referrer`, and `--http-header-fields` parameters for seamless 4K UHD HDR/DV HEVC playback.
- **Automated 4KHDHub End-to-End Verification**:
  - Added integration test `test_live_fourkhdhub_movie_resolution` in `tests/live_stream_verification.rs` verifying 4K UHD 2160p stream resolution and clean exit-code 0 playback in `mpv`.
- **Test Suite Isolation**:
  - Isolated watch history and favorites state in unit test `test_landing_deck_continue_watching_rendering` to prevent pollution from local disk history files.

### Security
- **Production Git & Pre-Push Hardening**:
  - Audited codebase for secrets, credentials, and user paths; verified zero sensitive data leaks.
  - Updated `.gitignore` with enterprise security rules, excluding large binary blobs (`npm/dist/mpv.exe`), `.npmrc`, `.env*`, and debug artifacts.
...
- **Native Android ARM64 Release Target & Pipeline**:
  ...
  - Configured automated packaging of `SumanMovies_Android_arm64.tar.gz` with native Bionic dynamic linking (`libc.so`), valid ELF `PT_PHDR` program header table, and `/system/bin/linker64` dynamic loader.
  ...
  - Updated universal installer script (`install.sh`) to detect Android Termux on 64-bit ARM and automatically fetch `SumanMovies_Android_arm64.tar.gz` with verified SHA256 checksums, enabling 1-second native installation without on-device compilation.
