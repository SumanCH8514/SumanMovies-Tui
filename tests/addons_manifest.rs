...
async fn test_invalid_addon_manifest_url_shows_actionable_error_and_preserves_state() {
    let client = sumanmovies_tui::providers::addons::client::AddonClient::new();
    let err = client
    ...
    let mut app = sumanmovies_tui::tui::app::App::new();
    let initial_addons_count = app.state().installed_addons.len();

    app.handle_action(sumanmovies_tui::tui::action::Action::SetStatus(format!(
    ...
    assert_eq!(notif.kind, sumanmovies_tui::models::NotificationKind::Error);
