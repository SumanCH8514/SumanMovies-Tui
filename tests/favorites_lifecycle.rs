...
fn test_favorites_path_is_outside_clear_cache_scope() {
    let favorites_path = sumanmovies_tui::config::favorites_path().expect("favorites path");
    let cache_dir = sumanmovies_tui::config::cache_dir();
    let data_dir = sumanmovies_tui::config::data_dir().expect("data dir");
...
async fn test_toggle_favorite_action_on_home_selected_result() {
    ...
    app.state_mut().active_screen = Screen::Home;
    app.state_mut().search_results = vec![sumanmovies_tui::tui::state::SearchResult {
