use sumanmovies_tui::providers::ReleaseProvider;
use sumanmovies_tui::providers::moviebox::client::MovieBoxClient;
...
async fn test_live_series_resolutions_and_streams() {
    ...
    let catalog =
        sumanmovies_tui::providers::moviebox::adapt::moviebox_search_json_to_catalog(&search_res);
    assert!(!catalog.is_empty(), "search should return catalog items");

    let series = catalog
        .iter()
        .find(|item| item.media_type == sumanmovies_tui::providers::models::MediaType::Series)
        .unwrap_or(&catalog[0]);
...
async fn test_live_moviebox_dynamic_movie_mpv_playback() {
    ...
    let movie = catalog
        .iter()
        .find(|item| item.media_type == sumanmovies_tui::providers::models::MediaType::Movie)
        .unwrap_or(&catalog[0]);
...
async fn test_live_fourkhdhub_movie_resolution() {
    let client = sumanmovies_tui::providers::fourkhdhub::FourKHdHubClient::new()
...
async fn test_live_moviebox_captions_end_to_end() {
    let service = sumanmovies_tui::service::MovieBoxService::new();
