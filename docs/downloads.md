...
If a MovieBox download is initiated without `yt-dlp` installed, SumanMovies-TUI prevents execution and displays an OS-tailored notification with installation guidance. Progressive streams from other providers (CircleFTP, DhakaFlix, 4KHDHub, Addons) do not require `yt-dlp` or `ffmpeg`.
...
- **Default path:** Files go to the user's OS download directory (`~/Downloads/SumanMovies-TUI`). On Android-family environments the code prefers shared `storage/downloads` when present.
- **Custom path:** Users can set a custom download directory in `/settings` → General → Download Folder or reset to default. Target directories are validated with a write probe before saving, and the code creates the `SumanMovies-TUI` subfolder hierarchy (`Movies/` and `Series/`). If custom storage becomes unavailable at runtime, the engine falls back to the default download location.
