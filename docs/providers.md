...
- Stream resolution in `providers/moviebox/mod.rs` resolves directly through `play-info/v2` with
  automatic deprecation notice URL filtering (`is_deprecation_notice_url`), preventing obsolete
  CDN placeholder videos from leaking into the stream selector when episodes lack valid streams.
