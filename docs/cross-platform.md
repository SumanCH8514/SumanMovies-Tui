...
- **Scoop Managed Environments**: Automatically detected via path markers (`/scoop/apps/`, `\scoop\apps\`, `/scoop/shims/`, `\scoop\shims\`). In-app binary overwrites are disabled to protect package manager integrity; the update modal displays Scoop instructions (`scoop update moviebox-tui`) with a dedicated `[s]` shortcut.
