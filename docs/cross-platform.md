...
- **Window Titles**: Contextual terminal emulator window titles are emitted dynamically
  reflecting active navigation mode and title (`SumanMovies-TUI — Streaming`,
  `SumanMovies-TUI — Live TV`, `SumanMovies-TUI — Addons`, `SumanMovies-TUI — {Title}`).
...
SumanMovies-TUI embeds an in-app binary upgrade engine (`src/updater/`) with cross-platform environment detection and deterministic fallback:
